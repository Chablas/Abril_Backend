    ...
    public class CorreoSalidaRecipientResolver : ICorreoSalidaRecipientResolver
    {
            ...
            IEnumerable<string>? baseCc = null)
        {
            ...
            try
            {
                ...
                // 3.b) Correos de los roles referenciados (1 query). Mismo camino que usa el aviso
                // a Tesorería para resolver "quien tenga el rol": user_role → person → workers.
                var roleIds = reglas
                    .Where(r => r.TipoCodigo == TipoRol && r.RoleId.HasValue)
                    .Select(r => r.RoleId!.Value)
                    .Distinct()
                    .ToList();

                var emailsByRoleId = new Dictionary<int, List<string>>();
                if (roleIds.Count > 0)
                {
                    var porRol = await (
                        ...
                        where roleIds.Contains(ur.RoleId) && ur.State && ur.Active
                           && w.EmailCorporativo != null && w.EmailCorporativo != ""
                        select new { ur.RoleId, Email = w.EmailCorporativo! }
                    ).Distinct().ToListAsync();

                    emailsByRoleId = porRol
                        .GroupBy(x => x.RoleId)
