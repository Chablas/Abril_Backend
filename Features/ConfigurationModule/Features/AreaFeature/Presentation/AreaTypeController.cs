...
{
    [ApiController]
    [Route("api/v1/area-type")]
    [Authorize]
    public class AreaTypeController : ControllerBase
    {
        private readonly IAreaTypeService _service;
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] AreaTypeCreateDto dto)
        {
            try
            {
                await _service.Create(dto);
                return Ok(new { message = "Tipo de área creado exitosamente." });
            }
        ...
        [HttpPut]
        public async Task<IActionResult> Update([FromBody] AreaTypeEditDto dto)
        {
            try
            {
                await _service.Update(dto);
                return Ok(new { message = "Tipo de área actualizado exitosamente." });
            }
            catch (AbrilException ex)
            {
                return BadRequest(new { message = ex.Message });
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                ...
                if (!result)
                    return NotFound(new { message = "Tipo de área no encontrado." });
                return Ok(new { message = "Tipo de área eliminado exitosamente." });
            }
            catch (AbrilException ex)
            {
                return BadRequest(new { message = ex.Message });
