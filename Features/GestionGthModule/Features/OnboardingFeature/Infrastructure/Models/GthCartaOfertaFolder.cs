...
{
    /// <summary>
    /// Carpeta única (singleton) de SharePoint donde se guardan las cartas oferta que Onboarding
    /// envía a los nuevos colaboradores (tabla <c>gth_carta_oferta_folder</c>). Se define por base de
    /// datos —no por appsettings— para poder cambiar la biblioteca destino sin redeploy: basta con
    /// actualizar <c>link_url</c> de la fila vigente. Existe a lo sumo una fila vigente
    /// (<c>state = true</c> y <c>active = true</c>).
    ///
