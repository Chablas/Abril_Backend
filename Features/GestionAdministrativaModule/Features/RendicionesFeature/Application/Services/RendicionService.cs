...
namespace Abril_Backend.Features.GestionAdministrativa.Rendiciones.Application.Services
{
    public class RendicionService : IRendicionService
    {
        ...
        {
            var (workerId, email, periodos) = await _repo.GetPeriodos(userId);

            var data = new RendicionFilterDataDto { Periodos = periodos };

            // A quién le llegan los dos correos que dispara la pantalla: el aviso a la jefatura y el
            // acuse de "Enviar a revisión". Salen de las MISMAS llamadas que hace el envío para que
            // la confirmación no anuncie un correo a alguien que la configuración dejó fuera. Va acá
            // y no con el listado porque es igual para toda la pantalla —está acotada a un solo
            // trabajador— y no cambia al mover los filtros.
            //
            ...
            if (workerId != null)
            {
                try
                {
                    data.CorreosEnvioRevision = await CorreosEnvioPrimeraRevision.ResolverAsync(
                        _correoResolver, _revisorResolver, workerId.Value, email);
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex,
        ...
        public async Task<RendirYEnviarResultDto> RendirYEnviarAPrimeraRevision(
            IReadOnlyCollection<int> solicitudIds, int userId)
        {
            // Rendir es de Gestión de Salidas (arma el PDF, lo sube y crea la planilla), con el guard
            // de que las salidas sean del propio usuario. El PDF no viaja a la pantalla: queda en
            // SharePoint y se abre desde Mis Rendiciones.
            var (_, rendidas, rendicionId, codigo) = await _gestionSalidaService.RendirYGenerarPlanilla(
                solicitudIds, userId, ownerUserId: userId);

            var resultado = new RendirYEnviarResultDto
            {
                RendicionId = rendicionId,
                Codigo      = codigo,
                Rendidas    = rendidas,
            ...
            // El envío es el MISMO camino que "Enviar a revisión" de Mis Rendiciones —guards, estado
            // y los dos correos—, para que las dos formas de enviar no puedan comportarse distinto.
            //
            // La rendición ya está guardada y no se deshace si el envío falla: el plazo para rendir es
            // lo que vence, no el envío. En ese caso la planilla queda "Lista para enviar" y el
            // mensaje lo dice, en vez de un error que invitaría a volver a rendir lo ya rendido.
            try
            {
            ...
            {
                _logger.LogWarning(
                    "Rendición {RendicionId} generada pero no enviada a primera revisión: {Motivo}",
                    rendicionId, ex.Message);
                resultado.Message =
                    $"La rendición {codigo} quedó lista, pero no se envió a revisión: {ex.Message} " +
                    "Puedes enviarla desde Mis Rendiciones.";
            }
            catch (Exception ex)
            {
                _logger.LogError(ex,
                    "Error enviando a primera revisión la rendición recién generada {RendicionId}", rendicionId);
                resultado.Message =
                    $"La rendición {codigo} quedó lista, pero no se pudo enviar a revisión. " +
