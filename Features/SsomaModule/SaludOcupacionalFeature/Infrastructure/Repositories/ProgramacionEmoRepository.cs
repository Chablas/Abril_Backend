...
namespace Abril_Backend.Features.Ssoma.SaludOcupacional.Infrastructure.Repositories
{
    public class ProgramacionEmoRepository : IProgramacionEmoRepository
    {
        ...
        public async Task<int> Create(ProgramacionCreateDto dto, int? userId)
        {
            ...
            // (contrato, onboarding, correos de EMO) la encuentra. La regla (lista, tope, reemplazo)
            // es la misma que al registrar el EMO sin programarlo: ver RazonSocialIngresoHelper.
            if (eligeRazonSocial && empresaVinculacion == null && dto.EmpresaId is int elegida)
                await RazonSocialIngresoHelper.AsignarAsync(ctx, worker, elegida, _reclutamiento, userId);
        ...
        public async Task<RazonesSocialesEmoDto> GetRazonesSociales(int? workerId)
        {
            ...
            if (worker != null)
            {
                ...
                if (worker.WorkersEstadoId == WorkersEstadoIds.FinalistaAprobado)
                    dto.RazonSocialActualId = worker.ContributorId;
            }
