...
namespace Abril_Backend.Features.MejoraContinuaModule.Features.LessonsDashboardFeature.Application.Services
{
    public class LessonsDashboardService : ILessonsDashboardService
    {
        ...
        public Task<LessonsDashboardDataDTO> GetData(DateTimeOffset? periodDate, int? userId, List<int>? lessonAreaIds, List<int>? projectIds, string? approvalStatus)
            => _repository.GetDataAsync(periodDate, userId, lessonAreaIds, projectIds, approvalStatus);
