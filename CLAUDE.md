...
- **D1**: Este proyecto tiene un solo entorno de base de datos real — no existe separación local/producción. `appsettings.Development.json` y `appsettings.Production.json` apuntan al mismo Postgres (mismo túnel SSH puerto 5544, mismo connection string, verificado el 2026-09-16). Cualquier SQL que se corre "en local" ya está en producción. Por lo tanto: mostrar el SQL final completo antes de correrlo, verificar con SELECT de solo lectura cuando se pueda, y no asumir que hay margen de error o un paso de "prueba en local" separado del real. Cambios destructivos (DELETE, DROP, UPDATE sin WHERE) requieren doble confirmación explícita antes de ejecutar.
- **D2**: Siempre usar `ON CONFLICT DO NOTHING` en los `INSERT` de `feature` y `role_feature`.
- **D3**: Para asignar roles usar `SELECT` en lugar de hardcodear el `feature_id` — el ID puede diferir entre entornos (aplica sobre todo a `features`/`role_feature`; para datos de negocio como `milestone`, ver D1).
- **D4**: Después de aplicar el SQL, el usuario debe cerrar sesión y volver a entrar para actualizar `allowed_features` en `localStorage`.
- **D5**: El connection string va en `appsettings.Development.json` (gitignored) — nunca en `appsettings.json`.
